...
int main(int argc, char** argv) {
  ...
  if (rank == src) {   
    ...
    //Receive One
    MPI_Recv(&oneReceived, 1, mpiOneStruct, dest, 0, MPI_COMM_WORLD, &status);
    ...
    //Receive Two
    MPI_Recv(&twoReceived, 1, mpiTwoStruct, dest, 0, MPI_COMM_WORLD, &status);
    ...
    std::cout << std::endl << std::endl;
    //Receive Three
    MPI_Recv(&threeReceived, 1, mpiThreeStruct, dest, 0, MPI_COMM_WORLD, &status);
  ...
  if(rank == dest){
    ...
    << "itemList: " << threeReceived.itemList << std::endl << "itemListSize: " 
    << threeReceived.itemListSize << std::endl << "warewolfPercentage: " << threeReceived.warewolfPercentage<< std::endl;
    //Changing the structs' instances' attributes
    //one
    for(int i = 0; i < 15; i++) oneReceived.list[i] =  -i*4;
    oneReceived.letter = 'j';
    oneReceived.size = 15;
    oneReceived.number = 4.2;
    oneReceived.x = 1756;
    oneReceived.y = 2323;
    
    //two
    twoReceived.playerlistSize = 30;
    for(int i = 0; i < twoReceived.playerlistSize; i++) twoReceived.playerlist[i] = i*6;
    strcpy(twoReceived.levelName,"anotherLevelNameExample");
    twoReceived.levelNameSize = sizeof(twoReceived.levelName);
    twoReceived.randVar = rand() % 133;
    twoReceived.gameBoardWidth = 42;
    twoReceived.gameBoardHeight = 42;
    
    //three
    strcpy(threeReceived.playerName,"another player's name");
    threeReceived.playerNameSize = sizeof(three.playerName);
    threeReceived.playerHealth = 0;
    threeReceived.playerMana = 0;
    strcpy(threeReceived.itemList,"itemlistIsFull");
    threeReceived.itemListSize = sizeof(threeReceived.itemList);
    threeReceived.warewolfPercentage = 0.1337;
    
    std::cout << "> " <<node_name<<" Sent to  node"<< src+1 << std::endl;
    //Send the structs' instances to the dest node
    MPI_Send(&oneReceived, 1, mpiOneStruct, src, 0, MPI_COMM_WORLD);
    MPI_Send(&twoReceived, 1, mpiTwoStruct, src, 0, MPI_COMM_WORLD);
    MPI_Send(&threeReceived, 1, mpiThreeStruct, src, 0, MPI_COMM_WORLD);
